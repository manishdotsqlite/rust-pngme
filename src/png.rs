...
impl PNG {
        ...
        fn from_chunks(chunks: Vec<Chunk>) -> Result<PNG, &'static str> {
                if chunks.is_empty() {
...
mod tests {
    ...
    // Helper function to create a valid chunk for testing
    fn create_test_chunk(chunk_type: &str, data: &[u8]) -> Chunk {
        let chunk_type = ChunkType::new(chunk_type).unwrap();
        Chunk::new(chunk_type, data.to_vec()).unwrap()
    }

    #[test]
    fn test_png_creation() {
        // Create a valid chunk
        let chunk = create_test_chunk("RuSt", b"Test data");

        // Create a PNG file with one chunk
        let png = PNG::from_chunks(vec![chunk]);
        assert!(png.is_ok());

        let png = png.unwrap();
        assert_eq!(png.chunks().len(), 1);
        assert_eq!(png.chunks()[0].chunk_type().bytes(), b"RuSt");
    }

    #[test]
    fn test_png_creation_with_empty_chunks() {
        // Attempt to create a PNG file with no chunks
        let png = PNG::from_chunks(vec![]);
        assert!(png.is_err());
        assert_eq!(png.unwrap_err(), "The PNG file must contain at least one chunk.");
    }

    #[test]
    fn test_append_chunk() {
        // Create a PNG file with one chunk
        let chunk = create_test_chunk("RuSt", b"Initial data");
        let mut png = PNG::from_chunks(vec![chunk]).unwrap();

        // Append a new chunk
        let new_chunk = create_test_chunk("tEXt", b"New data");
        png.append_chunk(new_chunk);

        // Verify the new chunk was appended
        assert_eq!(png.chunks().len(), 2);
        assert_eq!(png.chunks()[1].chunk_type().bytes(), b"tEXt");
    }

    #[test]
    fn test_remove_first_chunk() {
        // Create a PNG file with two chunks
        let chunk_1 = create_test_chunk("RuSt", b"Data1");
        let chunk_2 = create_test_chunk("tEXt", b"Data2");

        let mut png = PNG::from_chunks(vec![chunk_1, chunk_2]).unwrap();

        // Remove the first chunk ("RuSt")
        let removed_chunk = png.remove_first_chunk("RuSt");
        assert!(removed_chunk.is_ok());
        assert_eq!(removed_chunk.unwrap().chunk_type().bytes(), b"RuSt");

        // Verify the remaining chunk
        assert_eq!(png.chunks().len(), 1);
        assert_eq!(png.chunks()[0].chunk_type().bytes(), b"tEXt");
    }
    ...
    fn test_chunk_by_type() {
        ...
        let png = PNG::from_chunks(vec![chunk_1, chunk_2]).unwrap();

        // Find a chunk by type
        let found_chunk = png.chunk_by_type("tEXt");
        assert!(found_chunk.is_some());
        assert_eq!(found_chunk.unwrap().chunk_type().bytes(), b"tEXt");

        // Try to find a non-existent chunk
        let missing_chunk = png.chunk_by_type("zTXt");
        assert!(missing_chunk.is_none());
    }

    #[test]
    fn test_as_bytes() {
        // Create a PNG file with one chunk
        let chunk = create_test_chunk("RuSt", b"This is where your secret message will be!");

        let png = PNG::from_chunks(vec![chunk]).unwrap();

        // Serialize the PNG file into bytes
        let png_bytes = png.as_bytes();

        // Validate the header
        assert_eq!(&png_bytes[..8], PNG::STANDARD_HEADER);

        // Validate the chunk data
        let chunk_bytes = png.chunks()[0].as_bytes();
        assert_eq!(&png_bytes[8..], &chunk_bytes[..]);
    }

    #[test]
    fn test_parse_png_file() {
        ...
        // Parse the provided PNG_FILE byte array
        let parsed_png = PNG::from_bytes(&PNG_FILE[..]);

        // Ensure parsing succeeds
        assert!(parsed_png.is_ok());

        let png = parsed_png.unwrap();
        ...
        // Check for specific chunks (e.g., IHDR, IDAT, IEND)
